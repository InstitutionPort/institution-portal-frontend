...
export function Navbar() {
  const { data, isError, isLoading } = checkMe();
  const isLoggedIn = !!data?.data?.user && !isError && !isLoading; // avoid rendering stale login UI
  // const navBarHeightTailwind = `h-${navBarHeight}`
  return (

    <header className="flex items-center py-2 px-3">
      {/* Logo */}
