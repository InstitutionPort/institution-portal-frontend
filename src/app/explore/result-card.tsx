...
interface ResultCardProps {
  ...
  queryKey: [string, ExploreFilters]
}

export default function ResultCard({ result, queryKey }: ResultCardProps) {
  const [imageLoaded, setImageLoaded] = useState(false)
  const [imageError, setImageError] = useState(false)
  const NumberOfBadges = 5

  const getIcon = () => {
    switch (queryKey[1].mode) {
      case "Institution":
  ...
  return (
    ...
    <Card className="h-full hover:shadow-lg transition-shadow overflow-hidden">
      ...
      {queryKey[1].mode === "Institution" && result.image && (
      ...
      <CardHeader className={queryKey[1].mode === "Institution" && result.image ? "p-2" : "pb-3"}>
        {queryKey[1].mode !== "Institution" && (
          <div className="flex items-start justify-between">
            ...
            <FollowButton isFollowing={result.isFollowing || false} idToFollow={result.id} queryKey={queryKey} />
          </div>

        )}
        {queryKey[1].mode !== "Institution" && result.subtitle && (
