...
import ResultCard from "@/app/explore/result-card"
import EmptyState from "@/app/explore/empty-state"
...
import { useExploreFilterQuery, useExploreQuery } from "@/lib/queries/use-explore"
import SelectLocation from "../../components/select-location"
import { quartersInYear } from "date-fns/constants"
import { queryCustomFn } from "@/lib/queries/use-query-custom"
...
export default function ExploreContent() {
  ...
  const { data: modeFiltersRes, isLoading: filterIsPending, isError: filterIsError } = useExploreFilterQuery(filters.mode)
  const modeFilters = modeFiltersRes?.data

  // Fetch explore results
  const { data: exploreDataRes, isLoading, error, refetch: refetchExplore } = useExploreQuery(filters)
  const exploreData: ExploreResponse | undefined = exploreDataRes?.data as any
  ...
  useEffect(() => {
    ...
    if (exploreData && exploreData.currentPage < exploreData.totalPages) {
      ...
      const nextPageParams = setQueryParams(nextPageFilters)
      queryClient.prefetchQuery({
        queryKey: ["explore", nextPageFilters],
        queryFn: async () => await queryCustomFn(`/api/explore?${nextPageParams.toString()}`, true),
      });
    }
        ...
        {exploreData && exploreData.results.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {exploreData.results.map((result) => (
              <ResultCard key={result.id} result={result} queryKey={["explore", filters]} />
            ))}
          </div>
        )}
